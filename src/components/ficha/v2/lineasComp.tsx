...
// Goleadores (temporada): PJ · partidos con gol · goles/PJ · minutos por gol. El chip son los goles.
export function datosGoleadorTemp(j: any): ReactNode {
  return (
    <>
      ...
      {j.goles_pj != null && <span>{fmt2(j.goles_pj)} g/PJ</span>}
...
// Fantasy (temporada): media destacada + PJ. El chip son los puntos fantasy.
...
// ELO (temporada): máximo y mínimo de la temporada (sin icono natural → texto). El chip es el ELO actual.
...
export const leyGoleadorTemp = (<>{leyItem(<Balon size={12} />, 'valor: goles', 'var(--e4)')}{leyItem(<Escudo size={12} />, 'PJ')}{leyItem(null, 'con gol: partidos en los que marcó')}{leyItem(null, 'g/PJ: media de goles')}{leyItem(<Reloj size={12} />, 'minutos por gol')}</>)
...
export const leyFantasyTemp = (<>{leyItem(null, 'valor: puntos fantasy acumulados')}{leyItem(null, '⌀ media por partido')}{leyItem(<Escudo size={12} />, 'PJ')}</>)
export const leyEloTemp = (<>{leyItem(null, 'valor: ELO actual')}{leyItem(null, 'máx / mín de la temporada')}</>)
