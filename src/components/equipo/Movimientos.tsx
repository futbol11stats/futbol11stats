...
function Fila({ m }: { m: MovimientoRow }) {
  ...
  return (
    <li className="flex items-center gap-3 px-3 py-2 border-b border-pitch-700/50 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        <div className="text-sm font-medium text-white truncate">{m.nombre}</div>
...
function Bloque({ titulo, filas, inicial, discreto }: { titulo: string; filas: MovimientoRow[]; inicial: number; discreto?: boolean }) {
  ...
  return (
    <div className={discreto ? 'mt-4' : ''}>
      ...
      <ol className="bg-pitch-800 rounded-xl border border-pitch-700">
        {visibles.map((m, i) => <Fila key={`${m.codjugador}-${m.direccion}-${m.codtemporada}-${i}`} m={m} />)}
      </ol>
...
export default function Movimientos({ fichajes, promociones }: { fichajes: MovimientoRow[]; promociones: MovimientoRow[] }) {
  if (fichajes.length === 0 && promociones.length === 0) {
    return <p className="text-sm text-chalk-600">Sin altas ni bajas registradas.</p>
  }
  return (
    <div>
      <Bloque titulo="Altas y bajas" filas={fichajes} inicial={6} />
      <Bloque titulo="Promociones internas" filas={promociones} inicial={4} discreto />
    </div>
  )
}
