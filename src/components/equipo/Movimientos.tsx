...
import { fechaCortaYMD, tempLabel, type MovimientoRow } from '@/lib/equipo'
...
function Fila({ m, fichas }: { m: MovimientoRow; fichas: Record<string, string> }) {
  ...
  const canon = m.codjugador ? fichas[m.codjugador] : undefined
  const nombre = formatNombre(canon ?? m.nombre)
  const href = canon && m.codjugador ? jugadorHref(m.codjugador, canon) : null
  return (
...
function Bloque({ titulo, filas, inicial, discreto, fichas }: { titulo: string; filas: MovimientoRow[]; inicial: number; discreto?: boolean; fichas: Record<string, string> }) {
...
export default function Movimientos({ fichajes, promociones, fichas }: { fichajes: MovimientoRow[]; promociones: MovimientoRow[]; fichas: Record<string, string> }) {
